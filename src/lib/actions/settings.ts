...
import {
  ensureBusinessCategories,
  seedDefaultCategories,
} from "@/lib/supabase/seed-categories";
...
export async function updateSettings(input: AppSettings) {
  ...
  if (input.profile === "business") {
    // Kleinunternehmen gewählt: fehlende offizielle EÜR-Kategorien automatisch
    // ergänzen (idempotent — bestehende Kategorien bleiben unangetastet)
    await ensureBusinessCategories(supabase, user.id);
  } else {
    // Erster Speichervorgang eines neuen Vereins-Nutzers: Startkategorien anlegen
